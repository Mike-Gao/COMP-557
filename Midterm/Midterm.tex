...
\begin{center}
\includegraphics[width=16cm, height=11cm]{q4-diagram.png}
\end{center}

As shown above, a point light project point $p\in \mathbb{R}^3$ onto the plane with normal $n$ at $p'$. We let $b$ be the origin of the canonical basis and $n$ be the z-axis

$L_{ap} = L + (p-a)t$

$L_{nb} = b+nt$

We let $L_{ap} = L_{nb}$ and find their intersection point $c$. 

Case 1:
If $c$ is below $b$ (see figure above), then we can construct a new frame matrix $F$ by point $c$ and $-\vec{n}$

The coordinate of p is $F^{-1}p$, and its projection $p' = PF^{-1}p$, where P is the projection matrix from Q3. Using similarity transform, we can get $p'=FPF^{-1}p$ in the canonical basis.

Case 2:
If $c$ is above $b$ then we construct $F$ with $c$ and $\vec{n}$, we can get $p' = FPF^{-1}p$ in the canonical basis

Case 3:
If $c$ is at b, then $p' = b$
...
// FIX: Should use VinvT instead of V
// PositionForFP = vec4(VinvT,0) * M * VertexPosition
